...
void Port2_Init(void){
  ...
  // Use the port output register to put outputs to an initial state
    ...
    P2DIR  |= 0b11000100;//make pin 6 and 7 output
    P2OUT  &= 0b00111011;
    //MAKING Port 2 PM SEL=01
    P2SEL0 |= 0xC0;
    P2SEL1 &= ~0xC0;
...
void Port3_Init(void){
  ...
  // Use the port output register to put outputs to an initial state
    ...
    P3DIR |= 0b11000000; //setting pins 4 and 6 as outpus

    P3OUT &= 0b00111111; //Motors are set to sleep state
...
void Port6_Init(void){
  ...
  // Use the port output register to put outputs to an initial state
    ...
    //ULTRASONIC SENSOR
    P6DIR |= BIT2; //Trig making as output
    P6DIR &= ~BIT3; //echo making as input


    //P6OUT &= 0x0000;

    //Setting up PORT6 as GPIO
    P6SEL0 &= 0b11111001;
    P6SEL1 &= 0b11111001;
...
void Port8_Init(void){
  ...
  // Use the port output register to put outputs to an initial state
    ...
    //SERVO for sensor
    P8DIR |= BIT2;//setting as outputs

    //P8OUT &= 0x0000;

    //Setting PORT for PWM Setting
    P8SEL1 &=  ~BIT2;
    P8SEL0 |=  BIT2;
}
